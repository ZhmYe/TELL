...
// Conflict Graph Implementation, for Fabric++
type Graph = [][]int
type CG struct {
	graph    Graph
	txs      []*Transaction
...
func buildConflictGraph(txs []*Transaction) Graph {
	// Initialize adjacency matrix, default to all 0
	...
	for i, _ := range txs {
		...
		// Get the read-write set of the i-th transaction
		...
		for j, _ := range txs[:i] {
			// Get the read-write set of the jth transaction, j=0~i-1
...
// tarjan algorithm, to obtain all strongly connected components
